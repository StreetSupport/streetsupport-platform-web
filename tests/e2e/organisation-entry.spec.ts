...
test('should load homepage and check if h1 exists', async ({ page }) => {
  // Navigate to the homepage
  await page.goto('http://localhost:3000');  // Replace with the correct URL

  // Check if the h1 element exists and contains the expected text
  const heading = page.locator('h1');
  await expect(heading).toHaveText('Street Support Network');  // Updated expected text
});
